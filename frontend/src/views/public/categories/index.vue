      ...
      <v-col cols="12">
        ...
        <v-row v-else-if="categories.length > 0">
          <v-col cols="12" sm="6" md="4" lg="3" v-for="category in categories" :key="category.id">
            <v-card hover elevation="3" class="category-card" @click="filterByCategory(category.id)">
        ...
        <v-row v-else>
          <v-col cols="12" class="text-center">
            ...
            <p class="text-body-1 text-grey-lighten-1">Volte mais tarde quando houver categorias disponíveis.</p>
          </v-col>
        </v-row>
      </v-col>
    </v-row>
