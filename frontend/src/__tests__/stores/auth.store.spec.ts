...
describe('auth.store', () => {
  it('login sucesso e carrega perfil', async () => {
    ...
    vi.spyOn(apiClient, 'post').mockResolvedValueOnce({ data: { response: { access_token: 'jwt', expires_in: 3600 } } } as any)
    vi.spyOn(apiClient, 'get').mockResolvedValueOnce({ data: { response: { id: 1, login: 'user' } } } as any)
  ...
  it('login fallback em erro de rede', async () => {
    const auth = useAuthStore()
    vi.spyOn(apiClient, 'post').mockRejectedValueOnce({ code: 'ERR_NETWORK', message: 'Network Error' })
  ...
  it('register sucesso chama login', async () => {
    const auth = useAuthStore()
    vi.spyOn(apiClient, 'post').mockResolvedValueOnce({ data: { response: { id: 1, login: 'new' } } } as any)
    vi.spyOn(apiClient, 'post').mockResolvedValueOnce({ data: { response: { access_token: 'jwt', expires_in: 3600 } } } as any)
    vi.spyOn(apiClient, 'get').mockResolvedValueOnce({ data: { response: { id: 1, login: 'new' } } } as any)
